...
def kld_mod(p_mu, p_sigma, q_mu, q_sigma):

	kld = -0.5 * tf.reduce_mean(
			tf.reduce_sum(
				1 + tf.log(tf.square(p_sigma) + 0.00001) - tf.log(tf.square(q_sigma) + 0.00001)
				- (tf.square(p_mu - q_mu) + tf.square(p_sigma)) / tf.square(q_sigma), 1))
	return kld

def main(params):
	if params.input == 'PTB':
		# data in form [data, labels]
		input_data_folder_path = './DATA/'+params.inp_folder_name + '/'
		train_data_raw, train_label_raw = data_.ptb_read(input_data_folder_path)
		word_data, encoder_word_data,word_labels_arr, word_embed_arr, word_data_dict = data_.prepare_data(train_data_raw,train_label_raw, params,input_data_folder_path)

		train_label_raw, valid_label_raw, test_label_raw = label_data_.ptb_read(input_data_folder_path)
