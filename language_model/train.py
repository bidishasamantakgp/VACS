...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for epoch in range(FLAGS.max_epochs):
            ...
            for x, y in train_reader.iter():
                ...
                if count % FLAGS.print_every == 0:
                    ...
                    text_file.write('%6d: %d [%5d/%5d], train_loss/perplexity = %6.8f/%6.7f secs/batch = %.4fs, grad.norm=%6.8f \n' % (step,
                                                            epoch, count,
                                                            train_reader.length,
                                                            loss, np.exp(loss),
                                                            time_elapsed,
        ...
        print('Saved model', save_as)
        print("----------------------------------------------")
        print("Minimum Valid PPL is attained in epoch:%d and Validation PPL is %6.8f"%(minimum_vl_epoch,minimum_valid_ppl))
